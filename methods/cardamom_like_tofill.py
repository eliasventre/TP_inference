...
class NetworkInference:
    ...
    def fit(self, X_bin, X_prot, d_rna, d_prot, time,
            k0=0.0, k1=1.0,
            n_epochs=500, 
            lr=1e-2,
            weight_decay=0.0,
        ...
        """
        Learn θ by minimizing Loss(kon^θ(X_prot), X_bin)
        
        Args:
            X_bin: (n_cells, G) numpy (expected binary)
            X_prot: (n_cells, G) numpy (features)
        """

        device = "cpu"
        X_prot_tensor = torch.tensor(X_prot, dtype=torch.float32, device=device)
        X_bin_tensor = torch.tensor(X_bin, dtype=torch.float32, device=device)
